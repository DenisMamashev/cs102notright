...
def ego_network(
    ...
    if friends is False:
        fields: tp.List[tp.Dict[str, tp.Any]] = get_friends(user_id, fields=["nickname", "is_closed, deactivate"]).items
        friends = [i["id"] for i in fields if not (i.get("deactivate") or i.get("is_closed"))]
